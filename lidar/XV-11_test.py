...
# Modified by Olin College Hexapods Research Group
#
...
from vpython import *
from lidar_ros import *
import _thread, time, sys, traceback

use_real_robot = True # DEMO MODE IS BROKEN RIGHT NOW, USE THE REAL ROBOT OF SPOOF THE DATA FROM AN ARDUINO!
com_port = 'COM7'#'/dev/ttyACM0'  # 5 = COM6 on Windows, tty5 on Linux
baudrate = 115200
...
scene.forward = vector(0, 1, 0) #added vector
scene.background = vector(0.1,0.1,0.2)
ser = None


# setup display
point=points(pos=[(0,0,0) for i in range(360)], _size=5, color=vector(1,0,0))
pointb=points(pos=[(0,0,0) for i in range(360)], _size=5, color=vector(0.4,0,0))

point2=points(pos=[(0,0,0) for i in range(360)], _size=2, color=vector(1,1,0))
point2b=points(pos=[(0,0,0) for i in range(360)], _size=2, color=vector(0.4,0.4,0))
...
offset = 50
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=offset-1, thickness=1, color = color.yellow)
# plot ring every meter
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=6000, thickness=5, color = color.green)
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=1000, thickness=1, color = color.green)
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=2000, thickness=1, color = color.green)
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=3000, thickness=1, color = color.green)
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=4000, thickness=1, color = color.green)
ring(pos=vector(0,0,0), axis=vector(0,1,0), radius=5000, thickness=1, color = color.green)
# draw a line shows 0 degree
curve(pos=[(0,offset,0), (6000,offset,0)], radius=10)

label(pos = vector(0,2000,6000), text="Red : distance , Gray line: 0 degree\nYellow : quality + 50, yellow ring materializes 'quality == 0'\ndarker colors when quality is subpar")
label_speed  = label(pos = vector(0,500,-5000) , xoffset =1)
label_errors = label(pos = vector(0,800,-4000) , xoffset =1, text="errors: 0")
...
def update_view( angle, x, x1, x2, x3):
    global offset
    point.modify(angle, pos=vector( 0, 0, 0))
    pointb.modify(angle, pos=vector( 0, 0, 0))
    point2.modify(angle, pos=vector( 0, 0, 0))
    point2b.modify(angle, pos=vector( 0, 0, 0))
    ...
    if (x1 & 0x80): # flag for "bad data" ?
        ...
        node.publish(angle, dist_mm)
        # no, it's cool
        if dist_mm < 1000:
            # X+1:6 not set : quality is OK
            point.modify(angle, pos=vector( dist_mm*c,0, dist_mm*s))
            # point2.modify(angle, pos=vector( (quality + offset)*c,0, (quality + offset)*s))
    ...
    while True:
        try:
            ...
            if init_level == 0 :
                ...
                # speed
                b_speed = [ ord1(b) for b in ser.read(2)]

                # data
                b_data0 = [ ord1(b) for b in ser.read(4)]
                b_data1 = [ ord1(b) for b in ser.read(4)]
                b_data2 = [ ord1(b) for b in ser.read(4)]
                b_data3 = [ ord1(b) for b in ser.read(4)]
                ...
                # checksum
                b_checksum = [ ord1(b) for b in ser.read(2) ]
